...
function formatNamespaceFilterSchemaIssues(error: z.ZodError): string {
    return error.issues
        .map((issue) => {
...
export function loadNamespaceFilters(yamlContent: string): NamespaceFilterRule[] {
    ...
    if (!result.success) {
        throw new Error(`Invalid namespace-filters.yaml content: ${formatNamespaceFilterSchemaIssues(result.error)}`);
    }
